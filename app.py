...
#       audiofile = await message.download(f"\\{chat_id}\\audiofile.mp3")
#        audiofile = await message.download(f"audiofile{chat_id}.mp3")
    ...
    #) as w:
    #    w.write(reply)
    with open(os.path.join(path, f"downloads\\{chat_id}\\transcription.txt"), "w", encoding="utf-8") as w:
        w.write(reply)
